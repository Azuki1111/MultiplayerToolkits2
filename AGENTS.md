...
- 条件注册：`ActionCriteria`——`ConfigurationValueMatches` 按 GameConfiguration 值条件执行 SQL（**仅 InGame 侧、开局建库时求值**；`ModInUse inverse` 外部 mod 让位门控已全部废止——条目10/28调整用户裁决：不再考虑与 1.65/DMT/MPH 兼容）。
