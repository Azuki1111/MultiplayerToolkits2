...
| `InGame/WorldCongressAbstain/` | 条目27 世界议会弃权选项（全新功能，1.67/工坊无先例）：整文件替换 XP2 `WorldCongressPopup`（常规会/特别会投票共用此上下文，无通配 include 钩子；ReplaceUIScript 100000 + ImportFiles 100010，同名覆盖 xml 文件名必须保持原名）——**弃权 = 显式投 0 票**（条目27修复实证：引擎对「未投过票」的决议会随机代投，故 OnAccept 为弃权决议发 0 票 RESOLUTION_VOTE 标记已投票即不触发；提案侧原版全量发 0 票从不随机同款语义），唯一闸门 = UI 层 `CanMoveToNextPhase` 豁免 Abstained 项；决议卡/提案卡各加 AbstainButton（文本态切换），`UpdateVotingWidget` 早退锁定步进器，确认页显示「（弃权）」；stage4 补充提案页不加（原版有 PassButton）；**原版坑：OnLocalPlayerTurnEnd 置 nil 会期表但不销毁决议卡实例，ResumeCongress 重开界面旧卡可点必崩（原版步进器同款陷阱）——已补 m_kResolutionItemIM:ResetInstances() + 弃权回调 nil 守卫**；纯 UI 零 OOS 风险。开关收编条目12（WorldCongressAbstain_Show 自造 key 默认开，单通道 MPT_Settings_Toggle，关闭清空弃权标记还原原版）；文本仅 IG 注册（LOC_MPT_WC_ABSTAIN* 4 tag zh/en） |
