-- ===========================================================================
-- 条目22：商路总览面板（移植 1.67 BTS/UI/TradeOverview.lua）
-- 整文件替换原版 TradeOverview 上下文（ReplaceUIScript 100000 + ImportFiles 100010，
-- 原版文件 Base 单版本无 DLC 变体；同装压制 1.67 见 TradeSupport.lua 文件头）。
-- 三页签（我的商路/通往我城市的商路/可用商路）+ 分组视图 + 七级排序栏 + 按文明筛选 +
-- 真实收益双列 + 在途商路精确剩余回合（TradeSupport Tracker）+ 旅游业绩/贸易站标记。
-- 相对 1.67 的改动：
...
-- ==== 条目22改动4：1.67 发布态遗留 true（每次刷新刷十余行日志）改回 false
local dbug_print = false
...
--    routeInstance.BannerLighter:SetColor(brighterBackColor);
    ...
    -- ==== 条目22改动2：商人自动化剔除——1.67 此处为「取消自动化」按钮段
    -- （CancelAutomation 控件 + IsTraderAutomated/CancelAutomatedTrader 调用，1.67
    -- v1.34 起自动化从未激活，按钮恒隐），连同 XML 控件一并移除
...
function Open()
    ...
    -- ==== 条目22改动1（核心修复）：打开即清收益缓存——1.67 每回合只建一次缓存，
...
-- ==== 条目22改动5：OnSettingsButton 死函数剔除（指向已不移植的 BTS 设置面板）
...
function Initialize()
    ...
    -- ==== 条目22改动5：SettingsButton 注册剔除（1.67 指向已不移植的 BTS 设置面板）
    ...
    -- validity 类：宣战/议和（可用目的地集合）、商路容量（页签空余槽位/未用路线）、新城建立
    ...
    -- ==== 条目22改动5：Controls.SettingsButton:SetHide(true) 随控件一并删除
end
Initialize();
