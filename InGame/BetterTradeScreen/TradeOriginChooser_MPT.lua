-- ===========================================================================
-- 条目22：商人传送面板（移植 1.67 BTS/UI/Choosers/TradeOriginChooser.lua）
-- 整文件替换原版 TradeOriginChooser 上下文（ReplaceUIScript 100000 + ImportFiles 100010）。
-- 接口模式 TELEPORT_TO_CITY（商人「移动到己方城市中心」）滑出面板：横幅着色 + 城市字母
-- 序排序为 BTS 相对原版的增量；本文件相对 1.67 仅两处：
--   1. 剔除 1.67 遗留调试 print（AddCity/OnChangeOriginCityButton/OnChangeOriginCityFromOverview/
--      Initialize 四处无条件打印）；
--   2. 打开时经 LuaEvents.TradeRouteChooser_Close() 关闭目的地选择器（1.67 同款保留），
--      与条目22目的地选择器（本 mod 版）联动事件名不变。
-- ===========================================================================
-- ===========================================================================
--
...
function OnChangeOriginCityButton()
    if ( m_newOriginCity ~= nil and m_originCity ~= nil ) then
        ...
        -- ==== 条目22：1.67 遗留调试 print 剔除（原 print("cities are nil")）
    end
end
...
function OnChangeOriginCityFromOverview( city:table )
    if city ~= nil then
        ...
        if (m_AnimSupport:IsVisible()) then
            ...
            -- ==== 条目22：1.67 遗留调试 print 剔除（原 print("open sesame...")）
            OnOpen();
        end
    end
end
...
-- ===========================================================================
function Initialize()
    -- ==== 条目22：1.67 遗留调试 print 剔除（原 print("Initializing BTS Trade Origin Chooser")）
