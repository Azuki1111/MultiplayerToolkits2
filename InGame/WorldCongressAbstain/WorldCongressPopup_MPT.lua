...
-- ============================================================================
-- 条目27：世界议会投票弃权选项（联机工具箱 2.0 前端，本分区为本 mod 全部新增改动）
--
...
-- ============================================================================
local MPT_Show_Abstain:boolean = false; -- 设置开关当前值，LoadScreenClose 后由 MPT_SettingsPanel_ApplyAll 广播送达初值
-- 文本预加载缓存（无参数纯文本 tag，规约见 mod 开发规范）
local MPT_AbstainStr			:string = Locale.Lookup("LOC_MPT_WC_ABSTAIN");
local MPT_AbstainOnStr			:string = Locale.Lookup("LOC_MPT_WC_ABSTAIN_ON");
local MPT_AbstainTTStr			:string = Locale.Lookup("LOC_MPT_WC_ABSTAIN_TT");
local MPT_AbstainSummaryStr		:string = Locale.Lookup("LOC_MPT_WC_ABSTAIN_SUMMARY");

-- ===========================================================================
-- 条目27：刷新一张卡片弃权按钮的显示态（文本态切换：未弃权=「弃权」/已弃权=「已弃权」）
-- kInstance 卡片实例（决议卡或提案卡）；bAbstained 当前弃权标记。
-- stage4 补充提案卡（EmergencyProposalItem 模板）没有 AbstainButton，nil 守卫直接返回。
-- ===========================================================================
function MPT_RealizeAbstainButton(kInstance:table, bAbstained:boolean)
	if kInstance == nil or kInstance.AbstainButton == nil then
		return;
	end
	kInstance.AbstainButton:SetHide(not MPT_Show_Abstain);
	kInstance.AbstainButton:SetText(bAbstained and MPT_AbstainOnStr or MPT_AbstainStr);
	kInstance.AbstainButton:SetToolTipString(MPT_AbstainTTStr);
end

-- ===========================================================================
-- 条目27：全量刷新所有弃权按钮（重开界面/设置开关切换时调用）。
-- 决议卡仅 stage1 存活（stage2 时 m_kResolutionItemIM 已 Reset、实例引用失效）；
-- 提案卡 stage1/2 存活。
-- ===========================================================================
function MPT_RealizeAllAbstainButtons()
	if m_CurrentStage == 1 and m_kResolutionVotes then
		for _, kVoteData in pairs(m_kResolutionVotes) do
			if kVoteData.A and kVoteData.A.instance then
				MPT_RealizeAbstainButton(kVoteData.A.instance, kVoteData.Abstained);
			end
		end
	end
	if (m_CurrentStage == 1 or m_CurrentStage == 2) and m_kProposalVotes then
		for _, kVoteData in pairs(m_kProposalVotes) do
			MPT_RealizeAbstainButton(kVoteData.instance, kVoteData.Abstained);
		end
	end
end
...
function MPT_OnToggleAbstainResolution(kVoteData:table, kCostData:table)
	return function()
		...
		kVoteData.B.Abstained = bAbstained;
		if bAbstained then
			kVoteData.A.votes = 0;
			kVoteData.A.cost = 0;
			kVoteData.B.votes = 0;
			...
			if kChoiceData then
				kChoiceData.choice = -1;
				kChoiceData.target = -1;
				UpdateResolutionChoice(kChoiceData); -- 选项卡复位（A/B 卡槽熄灭、目标下拉隐藏）
			end
		end
		UpdateWorkingFavor();
		UpdateNavButtons();
		UpdateAllVotingWidgets(kCostData);
		MPT_RealizeAbstainButton(kVoteData.A.instance, bAbstained);
	end
end

-- ===========================================================================
-- 条目27：生成提案弃权切换回调（提案卡 AbstainButton 用）。
-- 置弃权：票数清零、方向复位；确认时随提案批量发送 0 票（引擎合法）= 弃权。
-- ===========================================================================
function MPT_OnToggleAbstainProposal(kVoteData:table, kCostData:table)
	return function()
		...
		UpdateNavButtons();
		UpdateAllVotingWidgets(kCostData);
		MPT_RealizeAbstainButton(kVoteData.instance, bAbstained);
	end
end

-- ===========================================================================
-- 条目27：条目12 设置面板开关广播（单通道 MPT_Settings_Toggle，规约同条目25）。
-- 关闭时清空全部弃权标记并恢复步进器可用，界面完全回到原版行为。
-- ===========================================================================
function OnMPT_Settings_Toggle(ParameterId:string, Value:boolean)
	if ParameterId == "WorldCongressAbstain_Show" then
		MPT_Show_Abstain = Value;
		if not Value then
			if m_kResolutionVotes then
				for _, kVoteData in pairs(m_kResolutionVotes) do
					kVoteData.Abstained = nil;
					if kVoteData.A then kVoteData.A.Abstained = nil; end
					if kVoteData.B then kVoteData.B.Abstained = nil; end
				end
			end
			if m_kProposalVotes then
				for _, kVoteData in pairs(m_kProposalVotes) do
					kVoteData.Abstained = nil;
				end
			end
			if m_IsInSession and m_kResolutionVotes and m_kProposalVotes then
				UpdateAllVotingWidgets(Game.GetWorldCongress():GetVotesandFavorCost(Game.GetLocalPlayer()));
			end
		end
		MPT_RealizeAllAbstainButtons();
...
function CanMoveToNextPhase()
	for _, kVoteData in pairs(m_kResolutionVotes) do
-- ==== 条目27：弃权的决议豁免必选校验（0 票提交，旧代码注释保留） ====
...
--			return false;
--		end
		if not kVoteData.Abstained then
			local choiceData:table = m_kResolutionChoices[kVoteData.Hash];
			if (kVoteData.A.votes + kVoteData.B.votes) <= 0 or not choiceData or (choiceData.choice < 0 or choiceData.target < 0) then
				return false;
			end
...
-- ---- 条目27 ----
	...
	for _, kVoteData in pairs(m_kProposalVotes) do
-- ==== 条目27：弃权的提案豁免必选校验（0 票提交，旧代码注释保留） ====
...
--			return false;
--		end
		if not kVoteData.Abstained and kVoteData.votes == 0 and not (kVoteData.voteBlocker and kVoteData.voteBlocker.NoUpvote and kVoteData.voteBlocker.NoDownvote) then
			return false;
		end
-- ---- 条目27 ----
	end
...
function PopulateResolutions()
	...
	for i, kResolutionData in pairs(kResolutions) do
		if type(i) == "number" then -- There's a "Stage" key kResolutions
			...
			-- 条目27：弃权按钮（决议卡标题行右侧；重入/热重载时经此处刷新按钮显示态）
			if instance.AbstainButton then
				instance.AbstainButton:RegisterCallback(Mouse.eLClick, MPT_OnToggleAbstainResolution(kVoteData, kCostData));
				MPT_RealizeAbstainButton(instance, kVoteData.Abstained);
			end
...
function UpdateVotingWidget(instance:table, kVoteData:table, kCostData:table, TestVoteCallback:ifunction, resolutionChoice:number)
	...
	-- 条目27：弃权项锁定票数步进器（取消弃权后经 UpdateAllVotingWidgets 恢复），tooltip 指向弃权说明
	if kVoteData.Abstained then
		instance.UpButton:SetDisabled(true);
		instance.DownButton:SetDisabled(true);
		instance.UpButton:SetToolTipString(MPT_AbstainTTStr);
		instance.DownButton:SetToolTipString(MPT_AbstainTTStr);
		instance.Label:SetText("0");
		instance.Label:SetToolTipString(MPT_AbstainTTStr);
	...
	for _, kProposal in pairs(kProposals) do
		...
		if canVote or (not m_IsInSession and m_ReviewTab == REVIEW_TAB_AVAILABLE_PROPOSALS) then
			...
			if not m_IsInSession and m_ReviewTab == REVIEW_TAB_AVAILABLE_PROPOSALS then
				...
				-- 条目27：弃权按钮（提案卡投票控件下方；重入/热重载时经此处刷新按钮显示态）
				if instance.AbstainButton then
					instance.AbstainButton:RegisterCallback(Mouse.eLClick, MPT_OnToggleAbstainProposal(kVoteData, kCostData));
					MPT_RealizeAbstainButton(instance, kVoteData.Abstained);
				end
	...
	if m_CurrentStage == 1 then
		...
		for i, kResolutionData in pairs(kResolutions) do
			if type(i) == "number" then -- There's a "Stage" key kResolutions
				...
				instance.Title:SetText(Locale.ToUpper(Locale.Lookup(kResolution.Name)));
-- ==== 条目27：弃权决议在确认页显示弃权状态（0 票、不显示选项/目标/描述，旧代码注释保留） ====
...
--				instance.Description:SetText(Locale.Lookup(kResolutionChoice.choice == 1 and kResolution.Effect1Description or kResolution.Effect2Description));
				if kVoteData.Abstained then
					instance.Status:SetText(MPT_AbstainSummaryStr);
					instance.TargetLabel:SetText("");
					instance.ChoiceLabel:SetText(MPT_AbstainSummaryStr);
...
function PopulateSummaryProposals(kProposals:table, kProposalIM:table)
	...
	for _, kProposal in pairs(kProposals) do
		...
		if not m_IsInSession or kVoteData then
			...
			-- 条目27：弃权提案在确认页显示弃权状态（票数/费用为 0，支持/反对图标组自动隐藏）
			if kVoteData ~= nil and kVoteData.Abstained then
				instance.Status:SetText(MPT_AbstainSummaryStr);
			end
...
function OnShutdown()	
	...
	LuaEvents.MPT_Settings_Toggle.Remove(OnMPT_Settings_Toggle); -- 条目27
