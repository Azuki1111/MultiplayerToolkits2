...
-- ============================================================================
-- [MPT 条目21] 政策卡收益显示（EPC = Extended Policy Cards, by Aristos）
-- 来源：联机工具箱 1.67 BRS/UI/GovernmentScreen.lua（三方 diff 验证 = 当前 2.0 原版
-- + ARISTOS 两块补丁，无版本漂移；原版无通配 include 钩子、无 DLC 完整变体，只能整文件覆盖，
-- 走条目15/17 模式：ReplaceUIScript 100000 + ImportFiles 100010）。
-- 功能：政策卡底部新增收益条（配合 GovernmentScreen.xml 同名覆盖新增的 EffectContainer/Effect
-- 控件），经 ExposedMembers.RMA.CalculateModifierEffect（本目录 MPT_RealModifierAnalysis.lua
-- 空 Context 提供）计算政策卡真实收益显示在卡面并追加进 tooltip。
-- 本文件相对 1.67 的改动（其余逐字节一致）：
--   ① isBRSActive 保持 1.67 的硬编码 true（RMA 随本 mod 内置必在；EPC 独立版此处为
--      Modding.IsModActive 探测 Infixo BRS GUID）
--   ② RealizePolicyCard 调用点加 RMA.CalculateModifierEffect nil 防御（1.67 无防御，
--      RMA 上下文加载失败时会崩整张政策卡填充）
-- ============================================================================
