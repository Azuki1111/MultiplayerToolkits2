...
VALUES
	('LOC_MPT_TIMER_MODE_NAME',				'zh_Hans_CN',	'回合计时器'),
	...
	('LOC_MPT_TIMER_MODE_TIERED_DESC',		'zh_Hans_CN',	'回合时间按回合节点表平滑线性过渡：默认 1~30 回合保持30秒，30~50 回合升至80秒，50~70 回合升至180秒，之后保持[newline][newline]节点与秒数可在 SmartTurnTimer_Tiers.sql 数据表中自定义'),
	('LOC_MPT_TIMER_CHAT_NAME',				'zh_Hans_CN',	'启用聊天指令'),
	('LOC_MPT_TIMER_CHAT_DESC',				'zh_Hans_CN',	'开启后聊天指令与快捷入口可用：[newline]p+ 加时20秒(每回合1次)[newline]p+++ 本回合无回合时间[newline]p-- 下回合减时15秒[newline][newline]聊天框旁 P++/P-- 按钮与 [ ] 热键同步显隐[newline]仅房主响应指令（关闭所有计时器模式下不生效）'),
