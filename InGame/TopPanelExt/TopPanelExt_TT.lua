-- ===========================================================================
-- 条目9：顶部面板扩展 Tooltip 渲染（移植自 1.67 TPE/UI/TopPanel_TT.lua）
-- 经 AddUserInterfaces(Context=InGame) 挂载 TopPanelExt_TT.xml 同名自动执行；
-- 绑定 TTManager 类型控件表，监听 LuaEvents.TopPanelToolTip_*_Refresh 刷新逐城市明细。
-- ===========================================================================
...
-- ===========================================================================
-- 文本预加载缓存（带参数 tag 拆为无参数 PRE/SUF + Lua .. 拼接）
-- ===========================================================================
local ProductionHeaderPRE	= Locale.Lookup("LOC_MPT_TPE_TOOLTIP_PRODUCTION_HEADER_PRE")	-- "+"
local ProductionHeaderSUF	= Locale.Lookup("LOC_MPT_TPE_TOOLTIP_PRODUCTION_HEADER_SUF")	-- " [Icon_ProductionLarge] 生产力"
local FoodHeaderPRE			= Locale.Lookup("LOC_MPT_TPE_TOOLTIP_FOOD_HEADER_PRE")			-- "+"
local FoodHeaderSUF			= Locale.Lookup("LOC_MPT_TPE_TOOLTIP_FOOD_HEADER_SUF")			-- " [Icon_FoodLarge] 食物"
local PopulationHeaderPRE	= Locale.Lookup("LOC_MPT_TPE_TOOLTIP_POPULATION_HEADER_PRE")	-- ""
local PopulationHeaderSUF	= Locale.Lookup("LOC_MPT_TPE_TOOLTIP_POPULATION_HEADER_SUF")	-- " [Icon_Citizen] 人口"

function OnTopPanelToolTip_Production_Refresh(Production_Info)
    m_TPT_TopPanel_Production_TT.Production_Header:SetText(ProductionHeaderPRE..Production_Info.TotalProduction..ProductionHeaderSUF)
...
function OnTopPanelToolTip_Food_Refresh(Food_Info)
    m_TPT_TopPanel_Food_TT.Food_Header:SetText(FoodHeaderPRE..Food_Info.TotalFood..FoodHeaderSUF)
...
function OnTopPanelToolTip_Population_Refresh(Population_Info)
    m_TPT_TopPanel_Population_TT.Population_Header:SetText(PopulationHeaderPRE..Population_Info.TotalPopulation..PopulationHeaderSUF)
