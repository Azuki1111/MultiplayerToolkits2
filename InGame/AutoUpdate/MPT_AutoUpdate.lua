...
--   2) MPT_AutoUpdate_EnableSelf() 确保本 mod 已启用（按本 mod GUID 匹配 Handle，不依赖订阅 ID；
--      本地 mod 无订阅 ID，1.67 按 SubscriptionId 匹配的 EnableMods 不适用）；
--   3) MPT_AutoUpdate_UpdateSelf() 退出到主菜单时触发本 mod 工坊更新（订阅 ID 常量
--      MPT_SUBSCRIPTION_ID 发布后填入；空串则跳过——当前为本地 mod 无订阅 ID）。
...
-- 本 mod 模组 ID（GUID，与 modinfo 一致）
local MPT_MOD_ID : string = "00000000-7369-4685-ab5f-bf77bc22b54e";
-- 本 mod 工坊订阅 ID（发布到创意工坊后填入；空串则退出时不触发更新）
local MPT_SUBSCRIPTION_ID : string = "";

-- ============================================================================
-- MPT_AutoUpdate_EnableSelf()
-- 确保本 mod 处于启用状态：遍历已安装 mod，按 Id（GUID）匹配 → Modding.EnableMod(handle, true)。
...
-- 订阅 ID 为空（本地 mod 未发布）时跳过；发布后填入 MPT_SUBSCRIPTION_ID 即生效。
-- ============================================================================
local function MPT_AutoUpdate_UpdateSelf()
	if MPT_SUBSCRIPTION_ID == "" then
		print("MPT 条目13：本 mod 无订阅 ID，退出时跳过更新检查");
		return;
	end
	Modding.UpdateSubscription(MPT_SUBSCRIPTION_ID);
	print("MPT 条目13：退出时触发本 mod 更新检查 (SubscriptionId=" .. MPT_SUBSCRIPTION_ID .. ")");
end
