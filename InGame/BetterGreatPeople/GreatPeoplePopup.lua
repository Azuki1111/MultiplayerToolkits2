...
--	  同字体，仅多 ColorSet，内联 [COLOR] 覆盖之）
--	②底部延伸背景 WoodPaneling2/3（XML 纯插入默认隐藏，开启时 Lua 显形 + 主背景
--	  高度 768→520；1.65 直接注释原版滚动条属不可运行时还原的纯装饰，不移植）
--	③招募竞争列表显示每回合点数增速 AmountPerTurn（联机抢伟人关键信息）
--	④往期招募列表倒序（1.65 用 StackGrowth Up，此处 XML 保持 Down 改 Lua 反序遍历，
--	  等价且可运行时还原）
...
--	英雄页签下延伸背景宽度不随 ResizeHeroPaneling 刷新（原版该函数只同步主背景），
--	纯装饰性残留。
-- ---- 条目29
...
-- ==== 条目29（BGP）：个人名颜色包裹——支持 mod 定义「名字 tag + _COLOR」后缀的专属
...
function ViewCurrent( data:table )
	...
	MPT_EraTipCurrent = nil;	-- ==== 条目31修复：清名册 Tooltip 去重键，刷新后下次悬停强制重建（招募状态可能已变）
...
--	增速为 0 时不显示）
	if Round(playerPoints.PointsPerTurn,1) > 0 then
		...
		instance.AmountPerTurn:SetText("");
		instance.AmountPerTurn:SetHide(true);
	end
-- ---- 条目29
	...
	instance.AmountPerTurn:SetColorByName( recruitColorName );	-- ==== 条目29（BGP）：增速与总量同色
...
--	走收口自愈（首帧文本布局未完成实测偏小，悬停期间后续回调以已布局控件实测收敛）
function MPT_FillGPEraTooltip( kPerson:table )
	...
	local sKey:string = tostring(kPerson.ClassID).."|"..tostring(kPerson.EraID);
