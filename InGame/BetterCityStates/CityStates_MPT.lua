-- ===========================================================================
-- 条目23：城邦界面增强（BCS = Better City States，移植 1.67 BCS/UI/CityStates_TPT.lua，
-- 布局见同目录 CityStates.xml）——任务行显示、单城邦视图默认「影响力」页、加成弹出区
-- 永久隐藏、宣战时使者排名条染红。整文件替换原版 CityStates 上下文（ReplaceUIScript
-- 100000 + ImportFiles 100010）；本文件 = 原版 Base + XP1 大使 + XP2 宗主加成三文件
-- 压平合并体（1.67 原样，同 1.67 ReplaceUIScript 机制要求）。
-- 相对 1.67 的增量 = 并入观察者 mod Better Spectator Mod（BSM）的城邦面板补丁，替代
-- 1.67「ModInUse_BSM criteria 切换 CityStates_SPEC.lua 双版本」机制，单文件直接兼容：
--   1. MPT_IsSpectator()：读 BSM 游戏属性 SPEC_NUM / SPEC_ID_k（未装 BSM 恒 false，
--      行为与原版完全一致）；
--   2. GetCityStatesMetNum / ViewList 行过滤：isHasMet 放宽为「或观察者」——观察者不
--      参与外交、未遇见任何城邦，不放宽则面板恒空（ViewCityState 左列 BSM 未放宽，
--      保真不补）；
--   3. GetData / AddInfluenceRow：CPL_ANONYMOUS（CPL 联赛匿名开关，仅 CPL 工具配置
--      该 Configuration 时激活，其余环境恒 nil 无副作用）时已遇见玩家名 / 宗主名显示
--      Anon_N（ GetData 仅匿名 HasMet 分支真实名，"YOU"/"UNKNOWN" 不动——BSM 原样）；
--   4. OnLocalPlayerTurnBegin：观察者面板隐藏时也持续 Refresh；
--   5. Initialize：去 CAPABILITY_CITY_STATES_VIEW 门控（BSM 观察者无此能力，门控拦截
--      面板初始化；SPEC_NUM 属性由 BSM Gameplay 脚本写入、时机晚于 UI 初始化，无法
--      用运行时观察者判定替代门控）。
-- 经 diff 核实不采纳的 BSM 改动（保真原则）：
--   ①OpenOverview/ViewList/GetData 注释掉 localPlayerID == -1 提前返回——BSM 观察者
--   占用真实玩家槽位（localPlayerID 有效），-1 门控本就不触发；且 MPT_IsSpectator 依赖
--   SPEC_ID_k 与 GetLocalPlayer() 相等，-1 时恒 false，「非观察者才返回」为无效补丁，
--   MPT 保留原版门控；
--   ②OnInputHandler 去掉 m_kScreenSlideAnim 关闭回调（OnClose）——ESC 关闭退化为
--   kAnim.Hide，丢失 Close() 的使节簿记与音效，Close() 自带 -1 防御与观察者无关；
--   ③GetData 重排 bonus 新达成检查丢失 m_kLastCityStates[iPlayer] ~= nil 保护（索引 nil
--   回归 bug，保留原版嵌套结构）；
--   ④BSM 自报 print 与注释态调试 print。
-- 城邦界面本体魔改（1.67 原有，原样保留）：AddCityStateRow 的 CUI 任务行块、
-- OpenSingleViewCityState 默认页、ViewList 的 BonusArea 隐藏、AddInfluenceRow 的宣战红条。
-- ===========================================================================
--[[
...
-- ===========================================================================
-- 条目23：观察者判定（并入 BSM 补丁，替代其 5 处内联 bspec 块）。BSM 的 Gameplay 脚本
-- Data/Spectator.lua 写入 SPEC_NUM / SPEC_ID_k 游戏属性；未装 BSM 时 SPEC_NUM 为 nil
-- 恒返回 false。注意 SPEC_ID_k 恒为真实玩家槽位号，故 localPlayerID == -1 时恒 false。
-- ===========================================================================
local function MPT_IsSpectator()
	if Game:GetProperty("SPEC_NUM") ~= nil then
		for k = 1, Game:GetProperty("SPEC_NUM") do
			if Game:GetProperty("SPEC_ID_"..k) ~= nil and Game.GetLocalPlayer() == Game:GetProperty("SPEC_ID_"..k) then
				return true;
			end
		end
	end
...
function GetCityStatesMetNum()
	...
	for _,kCityState in pairs(m_kCityStates) do
		-- 条目23：观察者视为已遇见全部城邦（BSM 补丁，原版行注释保留）
		--if kCityState.isHasMet then
		if kCityState.isHasMet or MPT_IsSpectator() then
			total = total + 1;
		end
	end
	return total;
...
function ViewList()
	...
	for iPlayer, kCityState in pairs( m_kCityStates ) do
		-- 条目23：观察者可见全部城邦行（BSM 补丁，原版行注释保留）
		--if kCityState.isHasMet then
		if kCityState.isHasMet or MPT_IsSpectator() then
...
function AddInfluenceRow(cityStateID:number, playerID:number, influence:number, largestInfluence:number)
	...
	-- 条目23：CPL 联赛匿名开关（BSM 补丁）——开启时使节行不显示真实文明名
	if GameConfiguration.GetValue("CPL_ANONYMOUS") == true then
		civName = "Anon_"..playerID;
	end
...
-- !!======================================================	给使者排名按照宣战情况修改颜色
	...
	if pLocalPlayer:GetDiplomacy():IsAtWarWith( playerID ) then
		kItem.AmountBar:SetColor(UI.GetColorValue(178/255,34/255,34/255,1));
	else
		kItem.AmountBar:SetColor(UI.GetColorValue(200/255,190/255,169/255,1));		
	end
--========================================================	
	return kItem;
end
		...
		if iPlayer ~= pLocalPlayer:GetID() and isCanReceiveInfluence then
			...
			if suzerainID ~=-1 then
				...
				elseif pLocalPlayer:GetDiplomacy():HasMet(suzerainID) then
					...
					-- 条目23：CPL 联赛匿名开关（BSM 补丁）——仅匿名 HasMet 分支的真实名，
					-- "YOU"/"UNKNOWN" 不动（BSM 原样）
					if GameConfiguration.GetValue("CPL_ANONYMOUS") == true then
						suzerainName = "Anon_"..suzerainID;
					end
				else
...
function OnLocalPlayerTurnBegin()
	...
	-- 条目23：观察者面板隐藏时也持续刷新（BSM 补丁，原版行注释保留）
	--if not ContextPtr:IsHidden() then
	if not ContextPtr:IsHidden() or MPT_IsSpectator() then
		Refresh();
	end
end
...
function Initialize()
	...
	-- 条目23：去能力门控（BSM 补丁）——观察者无 CAPABILITY_CITY_STATES_VIEW，门控会
	-- 拦截面板初始化；SPEC_NUM 属性由 BSM Gameplay 脚本写入、时机晚于 UI 初始化，无法
	-- 用运行时观察者判定替代。正常局城邦开启时该能力恒有，行为不变。
