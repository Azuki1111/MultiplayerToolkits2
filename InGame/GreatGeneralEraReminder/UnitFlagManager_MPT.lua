...
--   1.67 BCT/UnitFlagManager_BuilderCharges.lua（工人/军事工程师剩余劳动力显示，
...
--   所用；git 条目25 两次提交留痕），实验文件 EraBanner.xml/.lua 已删除。
...
--   与 BCT 4000——同装时其两上下文均不再加载，1.67 面板的 BER 开关经 TPT_Settings_Toggle
--   双通道监听继续生效，BCT 功能由本 mod 自带同名文件经 include 链供给）。
-- 开关：条目12 MPT_Settings 表 GreatGeneralEraReminder_Show（沿用 1.67 原 key，默认开），
--   LoadScreenClose 面板 ApplyAll 广播送达初值，勾选变化即时广播；
--   MPT_Settings_Toggle + TPT_Settings_Toggle 双通道监听（条目24 DPR 先例，与 1.67 设置互通）。
-- ===========================================================================
...
-- 不存在时 include 静默跳过。首环 BuilderCharges = 条目25 移植的 1.67 BCT 工人劳动力
-- 显示（VFS 同名文件，本 mod ImportFiles 注册）；BC 变体自身会 include 原版并叠加其
-- 覆写，本文件在其外再包一层，链序：本文件 → BCT → BC → 原版。
...
-- ===========================================================================
-- 条目25 开关广播（条目12 MPT_Settings 面板 + 1.67 TPT_Settings 面板双通道，同 key
--   GreatGeneralEraReminder_Show；LoadScreenClose ApplyAll 送达初值，勾选变化即时广播）
-- ===========================================================================
...
LuaEvents.TPT_Settings_Toggle.Add(OnMPT_Settings_Toggle);
